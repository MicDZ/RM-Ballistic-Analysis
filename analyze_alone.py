...
# 读取图片， 在图片上面标点
img = cv2.imread('/Users/micdz/Downloads/CamScanner 02-22-2024 20.48(1)_1.jpg')
...
def analyze_ballistic(points, dst_img):
    ...
    cv2.putText(dst_img, 'center', (x_mean, y_mean), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

    # 在左上角显示平均环数和平均距离
    cv2.putText(dst_img, 'score = %f' % (score), (0, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    cv2.putText(dst_img, 'distance = %f' % (distance), (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    cv2.putText(dst_img, 'hit_small = {}%'.format(hit_small * 100), (0, 150), cv2.FONT_HERSHEY_SIMPLEX, 1,
                (255, 255, 0), 2)
    cv2.putText(dst_img, 'hit_large = {}%'.format(hit_large * 100), (0, 200), cv2.FONT_HERSHEY_SIMPLEX, 1,
                (0, 100, 255), 2)
    cv2.putText(dst_img, 'hit_dart = {}%'.format(hit_dart * 100), (0, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255),
                2)
    ...
    return score, distance, hit_small, hit_large, hit_dart, dst_img

score, distance, hit_small, hit_large, hit_dart, dst_img = analyze_ballistic(points, dst_img)
cv2.imwrite('result.jpg', dst_img)